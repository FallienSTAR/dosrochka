...
def next_lose_phrase(chat_id: int) -> str:
    """Случайные фразы без повторов, пока не закончится весь список."""
    bag = _lose_bags.get(chat_id)
    if not bag:
        bag = SLOT_LOSE[:]
        random.shuffle(bag)
        _lose_bags[chat_id] = bag
    return bag.pop()
    ...
    def reply():  # ждём, пока барабаны докрутятся, чтобы не спойлерить результат
        try:
            if won:
                ...
                api("sendMessage", chat_id=chat_id, text=next_lose_phrase(chat_id), reply_markup=luck_keyboard())
...
def handle_message(msg: dict) -> None:
    ...
    if command in ("/start", "/app"):
        ...
        api(
            "sendMessage",
            chat_id=chat_id,
            text="Досрочка — планировщик погашения долгов: кредиты, кредитные карты, микрозаймы.\n"
                 "Нажмите кнопку ниже или «Досрочка» в меню слева от поля ввода.\n\n"
                 "Досрочка — калькулятор для самостоятельных расчётов, а не финансовая или юридическая "
                 "консультация. Суммы и даты приблизительные: сверяйтесь с графиком платежей и договором.",
...
def main() -> None:
    ...
    webhook = api("getWebhookInfo").get("url")
    if webhook:
        sys.exit(
            f"Бот уже работает в облаке (webhook: {webhook}).\n"
            "Запускать bot.py не нужно — иначе облачный бот перестанет получать сообщения."
        )
